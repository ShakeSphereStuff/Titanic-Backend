...
class TitanicAPI:
    class _Predict(Resource):
        ...
        def post(self):
            ...
            """     
            order = ['pclass','name','sex','age','sibsp','parch','fare','embarked','alone']
            # Get the passenger data from the request
            passenger = dict(request.form)
            if "alone" not in passenger.keys():
                passenger["alone"] = False

            passenger["age"] = int(passenger["age"])
            passenger["sibsp"] = int(passenger["sibsp"])
            passenger["fare"] = float(passenger["fare"])
            passenger["pclass"] = int(passenger["pclass"])
            passenger["parch"] = int(passenger["parch"])

            from collections import OrderedDict

            passenger = dict(map(lambda item: (item[0], [item[1]]), passenger.items()))
            passenger_ordered = OrderedDict()
            for key in order:
                passenger_ordered |= {key: passenger[key]}

            passenger = dict(passenger_ordered)
